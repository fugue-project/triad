...
_LLM_QA = [
    {
        "question": "What is Fugue Schema?",
        "answer": """Fugue Schema is a collection of pyarrow fields. It is a class
inside the `triad` package. It is used to describe the schema of any type of dataframe
that can be recognized by Fugue.""",
    },
    {
        "question": "How to create a Fugue Schema?",
        "answer": """There are many ways to construct a Fugue Schema. The most common
ways are:

```python
from triad import Schema

Schema("a:int,b:str") # from string
Schema([("a",int),("b",str)]) # from list of tuples of name and a primitive python type
Schema({"a":int,"b":str}) # from dict of names and primitive python types
Schema({"a":"int","b":"str"}) # from dict of names and Fugue DataType strings
```""",
    },
    {
        "question": "What is a Fugue Schema string?",
        "requires": ["fugue-data-type-string"],
        "answer": """A Fugue Schema string is a string expression in the format:
`<name>:<fugue data type string>[,<name>:<fugue data type string>]*`. For example:
...
""",
    ...
    {
        "question": "What is a Schema-like object?",
        "answer": """Any object that can be accepted by the Schema constructor is called
Schema-like object. Fugue API has a convention to take schema-like objects (usually
as a `schema` parameter), and convert automatically inside the functions.""",
    },
    {
        "question": "What is the difference between Fugue Schema and pyarrow Schema?",
        "answer": """They can be easily converted to each other. Fugue Schema supports
a subset of pyarrow data types.

```python
from triad import Schema

pyarrow_schema = Schema("a:int,b:str").pa_schema # Fugue-> pyarrow schema
fugue_schema = Schema(pyarrow_schema) # pyarrow -> Fugue schema
```
""",
    },
    {
        "question": "What is a partial or wildcard schema?",
        "answer": """A partial or wildcard schema is a schema with a wildcard `*` in
the string expression. It is used to describe a schema based on the input dataframe
schema, plus some columns generated by a transformation. It is very commonly used in
Fugue transform. For example:
...
def my_func(df:pd.DataFrame) -> pd.DataFrame:
    return df.assign(b="x", c=2)

df = pd.DataFrame(dict(a=[0,1]))

fa.transform(df, my_funct, schema="*,b:str,c:long")
```
""",
    },
]
